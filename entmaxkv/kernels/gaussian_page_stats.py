...
def _page_gaussian_stats_kernel(
    ...
    s_base = K_STD + batch_id * stride_kb + head_id * stride_kh
    ...
    for dim_start in range(0, head_dim, BLOCK_DIM):
        ...
        q_scaled = q * scale
        q_var_scaled = q * q * scale * scale

        ptrs = page_offsets[:, None] * stride_kp + dim_offsets[None, :] * stride_kd
        load_mask = page_mask[:, None] & dim_mask[None, :]

        k_mean = tl.load(k_base + ptrs, mask=load_mask, other=0.0).to(tl.float32)
        k_std = tl.load(s_base + ptrs, mask=load_mask, other=0.0).to(tl.float32)
...
def compute_page_gaussian_stats_triton(
    ...
    Returns:
        mu_scores_per_page, score_variance_per_page, mean_variance,
        variance_of_means, raw_mu_global. All outputs are float32.
    """
    if not q.is_cuda:
    ...
    q_c = q.contiguous()
    k_mean_c = k_mean[:, :, :pages_for_stats, :].contiguous()
    k_std_c = k_std[:, :, :pages_for_stats, :].contiguous()
